...
def show_result(x_, imgs, y_, num_epoch, show = False, save = False, path = 'result.png'):
    size_figure_grid = 3
    fig, ax = plt.subplots(x_.shape[0], size_figure_grid, figsize=(6, 6))
...
def imgs_resize(imgs, resize_scale = 286):
    ...
    for i in range(imgs.shape[0]):
        img = imresize(imgs[i], [resize_scale, resize_scale], Image.BICUBIC )
...
class data_loader:
    def __init__(self, root, batch_size=1, shuffle=False):
        ...
        img = cv2.imread(self.root + '/' + self.file_list[0])
        #img_[:,:,0] = img
        #img_[:,:,1] = img
        #img_[:,:,2] = img
    ...
    def next_batch(self):
        ...
        for i in range(self.flag, self.flag + self.batch_size):
            output[temp] = cv2.imread(self.root + '/' + self.file_list[i])
            img_float32 = np.float32(output[temp])
            output[temp] = cv2.cvtColor(img_float32, cv2.COLOR_BGR2RGB)
            temp = temp + 1
    ...
    def next_batch_rotate(self):
        ...
        for i in range(self.flag, self.flag + self.batch_size):
            ...
            img_float32 = np.float32(output[temp])
            output[temp] = cv2.cvtColor(img_float32, cv2.COLOR_BGR2RGB)
            output[temp]  = cv2.rotate(output[temp], cv2.ROTATE_90_CLOCKWISE)
            temp = temp + 1
