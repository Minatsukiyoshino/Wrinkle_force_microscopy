...
for iter in range(test_loader.shape[0]):
    ...
    cv2.imwrite(path, cv2.cvtColor(util.denorm(x_[0]).astype(np.float32),cv2.COLOR_BGR2GRAY) )
    ...
    cv2.imwrite(path, cv2.cvtColor(util.denorm(y_[0]).astype(np.float32),cv2.COLOR_BGR2GRAY) )
