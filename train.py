...
parser.add_argument('--L2_VGG', type=float, default=1e-3, help='lambda for L1 loss')
...
D_loss_positive_WGAN = D_positive_logits
D_loss_negative_WGAN = D_negative_logits
D_loss = (D_loss_positive + D_loss_negative) * 0.5
##########if WGAN##########
'''
D_loss = tf.reduce_mean(D_loss_negative_WGAN-D_loss_positive_WGAN )
alpha_dist = tf.contrib.distributions.Uniform(low=0., high=1.)
alpha = alpha_dist.sample((opt.batch_size, 1, 1, 1))
interpolated = y + alpha*(G-y)
inte_logit0,inte_logit = network.discriminator(x,interpolated, opt.ndf, reuse=True)
gradients = tf.gradients(inte_logit, [interpolated])[0]
grad_l2 = tf.sqrt( tf.reduce_sum(tf.square(gradients), axis=[1,2,3]))
gradient_penalty = tf.reduce_mean((grad_l2-1.)**2)
D_loss+=gradient_penalty*10
'''
###########################
...
#VGG_positive = VGG.VGG16(y, opt.ndf)
#VGG_negative = VGG.VGG16(G, opt.ndf, reuse=True)
#VGG_loss = opt.L2_VGG * tf.losses.mean_squared_error(VGG_positive, VGG_negative)
#G_loss_gan = tf.reduce_mean(-D_loss_negative_WGAN)
G_loss_L1 = tf.reduce_mean(tf.reduce_sum(tf.abs(G - y), 3))
G_loss_modified = tf.losses.mean_squared_error(y, G)
